...
“ES”:     (0.25,  12.50,  1.0,   15, time(14, 30)),  # S&P Emini
“NQ”:     (0.25,   5.00,  1.0,   15, time(14, 30)),  # Nasdaq Emini
“CL”:     (0.01,  10.00,  1.0,   15, time(14, 30)),  # Crude Oil
“GC”:     (0.10,  10.00,  1.0,   30, time(13, 30)),  # Gold
“EURUSD”: (0.0001, 1.00, 10000, 15, time(8,  0)),   # EUR/USD (standard lot)
“GBPUSD”: (0.0001, 1.00, 10000, 15, time(8,  0)),
“USDJPY”: (0.01,   1.00,  1000, 15, time(0,  0)),
“AUDUSD”: (0.0001, 1.00, 10000, 15, time(21, 0)),
}
...
def _compute_adx(self, df: pd.DataFrame) -> pd.Series:
    ...
    plus_di  = 100 * pd.Series(plus_dm).ewm(alpha=1/p,  adjust=False).mean() / atr
    minus_di = 100 * pd.Series(minus_dm).ewm(alpha=1/p, adjust=False).mean() / atr
